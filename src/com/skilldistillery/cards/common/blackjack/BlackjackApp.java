...
private void printResults(BlackjackHand ph, BlackjackHand dh) {
	...
	/*if dealer went over the limit, or if dealer ended with less points than player, player wins.
	 * 	- Else if statement is used to ensure that the player's final score is in bounds.
	 */
	else if((dh.getHandValue() > 21 || dh.getHandValue() < ph.getHandValue())) {
		System.out.println("\nYOU WON! Here is the final score!");
		resultString(ph, dh);
	}
	// if dealer is inbounds and has a higher score than the player who is also inbounds, dealer wins.
...
private void resultString(BlackjackHand ph, BlackjackHand dh) {
	...
	if ((hitStay.toLowerCase().equals("hit") || hitStay.toLowerCase().equals("h")) && (!hitStay.toLowerCase().equals("s") && !hitStay.toLowerCase().equals("stay"))) {
